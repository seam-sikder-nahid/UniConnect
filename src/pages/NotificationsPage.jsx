// src/pages/NotificationsPage.jsx — v2 with all types except messages
    ...
    <div className="space-y-4 pb-20 md:pb-4">
      <div className="flex items-center justify-between">
        <h1 className="page-header mb-0">
          Notifications
          {unreadCount > 0 && <span className="ml-2 text-primary-600">({unreadCount})</span>}
        </h1>
        {unreadCount > 0 && (
          <button onClick={markAllRead}
            className="flex items-center gap-1.5 text-xs font-medium text-primary-600 bg-primary-50 px-3 py-1.5 rounded-xl border border-primary-200 hover:bg-primary-100 transition-all">
          ...
          {filtered.map(notif => {
            ...
            return (
              <button key={notif.id} onClick={() => markRead(notif)}
                className={`w-full card text-left flex items-start gap-3 hover:shadow-md transition-all cursor-pointer ${
